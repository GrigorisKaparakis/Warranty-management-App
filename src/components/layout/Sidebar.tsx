  ...
  return (
    <>
      ...
      <aside className={`
        ...
        w-[280px] bg-slate-950 flex-shrink-0 flex flex-col h-screen 
        border-r border-white/5 transition-transform duration-300
        ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}
      `}>
        {/* Logo & User Info */}
        <div className="p-8 border-b border-white/5 flex items-center gap-4">
          <div className="w-12 h-12 bg-blue-600 rounded-2xl flex items-center justify-center font-black text-white italic text-xl shadow-xl shadow-blue-900/20">{logoText}</div>
          <div>
            ...
            <div className="mt-1">
              ...
              <span className="text-[8px] font-medium text-slate-600 uppercase tracking-widest block mt-0.5">WARRANTY H&K OS</span>
            </div>
          </div>
        </div>
        
        {/* Navigation Items: Παράγονται δυναμικά */}
        <nav className="flex-1 p-6 space-y-2 overflow-y-auto scrollbar-hide py-10">
          {dynamicMenu.map(item => (
            <NavLink 
              ...
              className={({ isActive }) => `
                w-full flex items-center gap-4 px-5 py-4 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] transition-all
                ${isActive ? 'bg-blue-600 text-white shadow-xl shadow-blue-900/20' : 'text-slate-500 hover:bg-white/5 hover:text-white'}
              `}
            >
        ...
        {/* Footer Sidebar */}
        <div className="p-6 border-t border-white/5 bg-slate-900/10 space-y-3">
          <button onClick={() => setIsChangePasswordModalOpen(true)} className="w-full py-3 bg-white/5 text-slate-500 font-bold text-[9px] uppercase rounded-xl hover:text-white hover:bg-white/10 tracking-widest transition-all">ΑΛΛΑΓΗ ΚΩΔΙΚΟΥ</button>
          <button onClick={() => AuthService.logout()} className="w-full py-3 bg-red-600/10 text-red-400 font-bold text-[9px] uppercase rounded-xl hover:bg-red-600 hover:text-white tracking-widest transition-all">ΑΠΟΣΥΝΔΕΣΗ</button>
        </div>
      </aside>
    </>
  );
