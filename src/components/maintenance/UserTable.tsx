              ...
              <tr key={u.uid} className={`hover:bg-slate-50/50 transition-colors ${u.disabled ? 'opacity-50' : ''}`}>
                <td className="px-8 py-6 font-bold text-slate-800">
                  <div className="flex items-center gap-3">
                    {u.email}
                    {u.uid === useStore.getState().user?.uid && (
                      <span className="bg-blue-100 text-blue-600 text-[8px] font-black px-2 py-0.5 rounded uppercase tracking-widest">ΕΣΕΙΣ</span>
                    )}
                    {u.disabled && (
                ...
                <td className="px-8 py-6">
                  ...
                  <select 
                    value={u.role} 
                    disabled={u.uid === useStore.getState().user?.uid}
                    onChange={(e) => FirestoreService.updateUserProfile(u.uid, { role: e.target.value as UserRole })} 
                    className="px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-[11px] font-black uppercase outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
