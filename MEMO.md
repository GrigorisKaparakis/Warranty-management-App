...
*   **`firebase/`**: (11 αρχεία)
    ...
    *   `chat.ts`: Υπηρεσία ζωντανής συνομιλίας και παρουσίας χρηστών.
    *   `stats.ts`: Διαχείριση συγκεντρωτικών μετρητών και στατιστικών Dashboard.
...
*   **Hybrid Pagination**: Χρήση Firestore `startAfter` για αποδοτική φόρτωση δεδομένων ανά σελίδες (Load More).
*   **Virtual Rendering**: Ενσωμάτωση `react-virtuoso` για απροβλημάτιστη προβολή λιστών με χιλιάδες εγγραφές.
...
*   **Dashboard Settings**: Διαμόρφωση εμφάνισης Dashboard και εργαλείο Επαναϋπολογισμού Μετρητών (Stats Sync).
...
### 📅 ΕΚΔΟΣΗ 1.3.x (ΣΧΕΔΙΑΣΜΟΣ)
1.  **Dark Mode**: Υποστήριξη σκοτεινής εμφάνισης για χρήση σε περιβάλλοντα συνεργείου.
2.  **Excel/CSV Export**: Εξαγωγή φιλτραρισμένων λιστών για λογιστική χρήση.
3.  **Push Notifications**: Ειδοποιήσεις για αλλαγές status και νέα μηνύματα στο Chat.
